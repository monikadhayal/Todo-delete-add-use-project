      ...
      <div className="bg-white w-96 rounded-b-lg ">
        <div className="pl-5">
          {todoList.map((item) => (
            ...
            <div className="w-80 px-4 py-2 rounded-lg bg-white text-black border-l-[6px] border-l-blue-600 hover:shadow-[inset_6px_0_10px_#3b82f6] transition-all duration-500 flex justify-between" key={item.id}>
              {item.todoItem}
              <span onClick={() => {deleteTodoitem(item.id)}}> <DeleteIcon/> </span>
            </div>
          ))}
        </div>
